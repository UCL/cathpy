...
class SsapResult:
    """
    Provides a data store for a SSAP result
    ...
    @classmethod
    def from_string(cls, ssap_line, *, cath_version='current'):
    ...
    def set_redis(self, redis, key=None):
        """
        Sets the SSAP result in the provided Redis datastore

        Args:
        * redis (Redis): redis datastore object
        """
        if not key:
            key = self.unique_key()
        v = self.to_dict()
